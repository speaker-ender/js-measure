...
const NotificationTray: React.FC<INotificationTray> = (props) => {
    ...
    const wrapperRefs = useMemo(() => !!notifications && notifications.map(() => createRef<HTMLDivElement>()), [notifications, isClient]);
    const notificationRefs = useMemo(() => !!notifications && notifications.map(() => React.createRef<HTMLDivElement>()), [notifications, isClient]);
